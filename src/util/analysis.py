...
class DataAnalysis:
    ...
    def show_distributions(self, filename="distributions.png",
        ...
        for index, feature in enumerate(feature_names):
            sns.histplot(self.x, x=feature, kde=True, ax=axs[int(index / 2), index % 2])
    ...
    def show_joint(self, on, filename="joint.png", title="Joint"):
        ...
        p = sns.jointplot(data=self.x, x=x, y=y, hue=z, legend=False)
        p.plot_joint(sns.scatterplot, zorder=0)
        p.plot_marginals(sns.rugplot, height=-.15, clip_on=False)
        p.plot_marginals(sns.kdeplot)
        p.fig.suptitle("{} - {} - {} ".format(x, y, z) + title)

        if self.save_file:
            p.savefig(os.path.join(self.image_folder, "{}:{}:{}_".format(x, y, z) + self.filename_prefix + filename))
    ...
    def show_line(self, on, filename="line.png", title="Line"):
        ...
        p = sns.relplot(x=x, y=y, kind="line", hue=z, col=w, col_wrap=None if w is None else 10, estimator=None,
                        data=self.x, legend="full")

        if p is not None:
            if self.save_file:
                p.savefig(os.path.join(self.image_folder, "{}:{}:{}:{}_".format(x, y, z, w)
